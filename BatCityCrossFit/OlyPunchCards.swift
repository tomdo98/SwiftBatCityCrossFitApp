...
class OlyPunchCards: UIViewController {
    ...
    @IBAction func RechargeButton(_ sender: Any) {
        ...
        refreshAlert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { (action: UIAlertAction!) in
            ...
            self.ref.child("oly_punch_cards").queryOrdered(byChild: "username").queryEqual(toValue: self.mUserLoggedIn).observeSingleEvent(of: .value, with: {(Snap) in
                if let snapDict = Snap.value as? [String:AnyObject]{
                    for each in snapDict{
                        ...
                        let key1 = (each.0)
                        print(key1)
                        let userRef = self.ref.child("oly_punch_cards").child(key1)
                ...
                {
                    self.oly_punch_cards(data: [Constants.oly_punch_cards.username : self.mUserLoggedIn], user: self.mUserLoggedIn, punchesleft: "9", last_updated: todaysDate)
                    self.MarkUsed(Punches: "9")
                }
            })
    ...
    @IBAction func PunchButtonAction(_ sender: Any) {
        ...
        ref.child("oly_punch_cards")
            ...
            .observeSingleEvent(of: .value, with: {(Snap) in
                ...
                if let snapDict = Snap.value as? [String:AnyObject]{
                    self.card_stat(data: [Constants.card_stat.stat_type: "use_punch_oly"], lastupdated: todaysDate)
                    for each in snapDict{
                        let key1 = (each.0)
                        let userRef = self.ref.child("oly_punch_cards").child(key1)
    ...
    func getcurrentpunches()
    {
        ...
        var pleft:String!
        ref.child("oly_punch_cards").queryOrdered(byChild: "username").queryEqual(toValue: mUserLoggedIn).observeSingleEvent(of: .value, with: {(Snap) in
    ...
    func oly_punch_cards(data: [String:String],user: String, punchesleft: String, last_updated: String  )
    {
        var mdata = data
        mdata[Constants.oly_punch_cards.punchesleft] = punchesleft
        mdata[Constants.oly_punch_cards.last_updated] = last_updated
        mdata[Constants.oly_punch_cards.username] = mUserLoggedIn
        
        ref.child("oly_punch_cards").childByAutoId().setValue(mdata)
    }
}
